...
def test_inputs(config):
    ...
    if not config['time_steps'].isdigit() or int(config['time_steps']) <= 0:
        raise ValueError(
            "Time_steps must be a positiv integer")  # filters out non integer numbers, negativ numbers and 0
    if not config['future_steps'].isdigit():
        raise ValueError(
            "Future_steps must be a positiv integer")  # filters out non integer numbers and negativ numbers
    if not config['num_samples'].isdigit() or int(config['num_samples']) <= 0:
        raise ValueError(
            "Num_samples must be a positiv integer")  # filters out non integer numbers, negativ numbers and 0
    ...
    if not config['train_test_ratio'].isnumeric():
        raise ValueError(
            "Train_test_ratio must be a positiv integer")  # filters out non numeric numbers and negativ numbers
    ...
    if not config['learning_rate'].isnumeric() or int(config['learning_rate']) <= 0:
        raise ValueError(
            "Learning_rate must be a positiv number")  # filters out non numeric numbers and negativ numbers
    ...
    if not config['steps_to_predict'].isdigit() or int(config['time_steps']) <= 0:
        raise ValueError(
            "Steps_to_predict must be a positiv integer")  # filters out non integer numbers, negativ numbers and 0
...
def dialog_load_config():
    ...
    values = [
        'time_frame_start', 'time_frame_end', 'n_steps', 'time_steps', 'future_steps', 'num_samples', 'noise_level',
        'train_test_ratio', 'model', 'custom_circuit', 'circuit', 'epochs', 'batch_size', 'learning_rate',
        'loss_function', 'steps_to_predict'
    ]
    msg = ""
    while True:
